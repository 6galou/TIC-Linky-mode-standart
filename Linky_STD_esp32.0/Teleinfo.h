...
// libraries
    ...
    #define TI_Debugf(...) Serial.print(__VA_ARGS__)
...
#define TINFO_HT  0x09 // HT (separation dans mode standart )
...
class TInfo
{
  public:
    ...
    ValueList *   addCustomValue(char * name, char * value, char * date, uint8_t * flags);
    ValueList *   getList(void);
    uint8_t       valuesDump(void);
    char *        valueGet(char * name, char * value);
    boolean       listDelete();
    unsigned char calcChecksum(char *etiquette, char *valeur, char *date) ;

  private:
    void          clearBuffer();
    ValueList *   valueAdd (char * name, char * value, char * date, uint8_t checksum, uint8_t * flags);
